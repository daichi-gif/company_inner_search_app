...
def display_sidebar_content():
    ...
    if st.session_state.mode == ct.ANSWER_MODE_1:
        st.markdown("### 📋 社内文書検索について")
        st.info("入力内容と関連性が高い社内文書のありかを検索できます。")
        st.markdown("### 💡 入力例")
        st.code("社員の育成方針に関するMTGの議事録", language=None)
        st.code("マーケティング戦略について", language=None)
        st.code("人事部の議事録", language=None)
    else:
        st.markdown("### 💬 社内問い合わせについて")
        st.info("質問・要望に対して、社内文書の情報をもとに回答を得られます。")
        st.markdown("### 💡 入力例")
        st.code("人事部に所属している従業員情報を一覧化して", language=None)
        st.code("来年の目標について教えて", language=None)
        st.code("新入社員の研修について詳しく", language=None)
    
    st.markdown("---")
    st.warning("基本的にお入力されたお問い合わせの回答を得られます。")


def display_sidebar_menu():
    """
    サイドバーのメニュー表示（ラジオボタン形式）
    """
    with st.sidebar:
        st.markdown("## 利用目的")
...
def display_warning_message():
    ...
    ">
        ...
        <span style="color: #333; font-weight: 500;">具体的に入力したほうが精確な回答を得やすいです。</span>
    </div>
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            # ページ番号を取得（0始まりの可能性があるため、1始まりに補正）
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                # ページ番号が取得できない場合のための分岐処理
                if "page_number" in sub_choice:
                    # ページ番号は0始まりの可能性があるため、1始まりに補正
                    page_number = int(sub_choice["page_number"]) + 1
